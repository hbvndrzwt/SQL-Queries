...
(
	SELECT  a.*,
			CASE WHEN YEAR(ExpertCertifiedDate) = [Year] THEN 'ExpertCertified' ELSE
			CASE WHEN YEAR(AdvancedCertifiedDate) = [Year] THEN 'AdvancedCertified' ELSE
			CASE WHEN YEAR(RapidCertifiedDate) = [Year] THEN 'RapidCertified' 
			ELSE  'NotCertified' END END END AS Certification
	FROM
	(
		SELECT  UserId,
				MAX(IsDeveloper) AS IsDeveloper,
...
SELECT  *
INTO community.Product_Existing_Users_Year
